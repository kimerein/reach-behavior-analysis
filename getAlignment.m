...
[X,Y,D]=alignsignals(temp1,temp2,50); 
if settings.isInSecondHalf==true
    D=D-midLength;
...
function out=throwOutOnStretches(data, times)
...
while any(indon) && safeguard<length(data)
    fi=find(temp>0.5,1,'first'); % find first on
    fi2=find(temp(fi:end)<0.5,1,'first'); % find next off
    if fi2<threshInds
